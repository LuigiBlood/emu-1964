...
#ifdef _DEBUG
	char *loadstr = gfx->gbi0matrix.load?"Load":"Mul";
	char *pushstr = gfx->gbi0matrix.push?"Push":"Nopush";
...
void RSP_GBI1_Line3D(Gfx *gfx)
{
	...
	else
	{
		do {
			...
			//Tri #2
			DEBUG_DUMP_VERTEXES("Line3D 2/2", dwV2, dwV3, dwV0);
			bTrisAdded |= CRender::g_pRender->AddTri(dwV2, dwV3, dwV0);
