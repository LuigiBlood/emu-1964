...
void ClipVertexesOpenGL()
{
	...
	for( int i=0; i<g_clippedVtxCount; i++ )
	{
		TLITVERTEX &v =  g_clippedVtxBuffer[i];
		v.x = (v.x - gRSP.vtxXAdd)/gRSP.vtxXMul;
		v.y = (v.y - gRSP.vtxYAdd)/gRSP.vtxYMul;
		v.z = v.z*2-1.0f;
		g_vtxProjected5Clipped[i][0] = v.x/v.rhw;
		g_vtxProjected5Clipped[i][1] = v.y/v.rhw;
		g_vtxProjected5Clipped[i][2] = v.z/v.rhw;
		g_vtxProjected5Clipped[i][3] = 1/v.rhw;
